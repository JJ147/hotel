...
public interface HotelMapper {
     ...
     * @return
     */
    @Select("insert into `student` (stuId,stuName,stuPwd,grade,stuSex,myClass,academy,professional) values(#{stuId},#{stuName}#{stuPwd},#{grade},#{stuSex},#{myClass},#{academy},#{professional})")
    Integer addStudent(StudentRegisterModel studentRegisterModel);

    @Select("select *from `student` where stuId =#{stuId}")
    StudentRegisterModel getStudentById(@Param("stuId") Integer stuId);
}
